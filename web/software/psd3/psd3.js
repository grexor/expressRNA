...
psd3.Pie.prototype.draw = function(svg, totalRadius, dataset, originalDataset, originalDatasetLength, innerRadius, outerRadius, radiusDelta, startAngle, endAngle, parentCentroid) {
    ...
    // gregor
    psd3.Pie.prototype.arcColor = function(d) {
        return _this.config.color(d);
    };
    ...
    var dblclick = function(d) {
      // Gregor: do not extend outer arcs
        if (d.data.inner==undefined) return;
        _this.reDrawPie(d, originalDataset);
        var event = new CustomEvent('pie_double_click', { bubbles: true, detail: { env: d.data.label, level: originalDataset.length}});
        this.dispatchEvent(event);
    };

    var mouseover = function(d) {
        var event = new CustomEvent('pie_mouseover', { bubbles: true, detail: { cid: d.data.cid}});
        this.dispatchEvent(event);
    };

    var mouseout = function(d) {
        var event = new CustomEvent('pie_mouseout', { bubbles: true, detail: { cid: d.data.cid}});
        this.dispatchEvent(event);
    };
    ...
    var arcs = svg.selectAll("g." + clazz)
        ...
        // commented out, we directly fill the arc color in the path
        //.attr("color", _this.arcColor)
        .attr("transform",
    ...
    //console.log(arcs[0].length + ":" + _this.arcIndex);
    //for (var i=0; i<arcs[0].length; i++) {
    //  console.log(arcs[0][i]);
    //}
    ...
    var paths = arcs.append("path")
        .attr("fill", _this.arcColor) // Gregor
    ...
    var texts = arcs.append("text")
        ...
        .attr("transform", function(d) {
            ...
            //if(_this.config.rotateLabel === true){
                ...
                position = rotateAngle % 360;
                if ( (position>90) && (position<270) )
                  rotate2 = "rotate(180, " + b + ")";
                else
                  rotate2 = "";
            //}
            return "translate(" + a + ")" + rotate + rotate2;
        })
